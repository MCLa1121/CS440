    ...
    extends Object
{
        ...
        public Pair<Matrix, Matrix> next()
        {
            ...
            try
            {
                XBatch = this.getFullX().getSlice(rIdxStart, rIdxEnd,
                                                  0, this.getFullX().getShape().numCols());
                YGtBatch = this.getFullYGt().getSlice(rIdxStart, rIdxEnd,
                                                      0, this.getFullYGt().getShape().numCols());
            } catch(Exception e)
            {
    ...
    public void shuffle()
    {
        try
        {
            ...
            int randIdx = -1;
            Matrix tmp = null;
            for(int idx = 0; idx < this.getFullX().getShape().numRows(); ++idx)
            {
                randIdx = this.getRandom().nextInt(idx + 1);

                // swap row in X
                tmp = this.getFullX().getRow(randIdx);
                this.getFullX().copySlice(randIdx, randIdx + 1, 0, this.getFullX().getShape().numCols(),
                                          this.getFullX().getRow(idx));
                this.getFullX().copySlice(idx, idx + 1, 0, this.getFullX().getShape().numCols(),
                                          tmp);

                // now swap row in YGt
                tmp = this.getFullYGt().getRow(randIdx);
                this.getFullYGt().copySlice(randIdx, randIdx + 1, 0, this.getFullYGt().getShape().numCols(),
                                            this.getFullYGt().getRow(idx));
                this.getFullYGt().copySlice(idx, idx + 1, 0, this.getFullYGt().getShape().numCols(),
                                            tmp);
            }
        } catch(Exception e)
        {
    ...
    public long size()
    {
        final long numSamples = this.getFullX().getShape().numRows();
