...
import edu.bu.labs.cp.game.Game;
import edu.bu.labs.cp.utils.Triple;


import labs.cp.ReplayBuffer;
import labs.cp.Dataset;
    ...
    extends Object
{
                              ...
                              Namespace ns)         // namespace of command line args
    {
        ...
        Matrix X = trainingData.first();
        Matrix YGt = trainingData.second();
        ...
        for(int epochIdx = 0; epochIdx < numUpdates; ++epochIdx)
        {
            ...
            while(it.hasNext())
            {
                ...
                {
                    // get predictions
                    Matrix YHat = qFunction.forward(batch.first());

                    // a pytorch-esque gradient descent update api
                    opt.reset();
                    qFunction.backwards(batch.first(),
                                        lf.backwards(YHat, batch.second()));
                    opt.step();
                } catch(Exception e)
                {
                    e.printStackTrace();
    ...
    public static void main(String[] args)
    {
        ...
        for(int cycleIdx = 0; cycleIdx < numCycles; ++cycleIdx)
        {
            ...
            double avgUtil = expectedUtilityAndAvgGameLength.first();
            double avgGameLength = expectedUtilityAndAvgGameLength.second();
